...
<div class="sidebar">
  <div class="">
    <ul class="post-list ui-list-unstyled">
      <li v-for="p in posts">
        <router-link :to="{name: 'post-detail', params:{detailId: p.fileName}}" exact>{{p.title}}</router-link>
        <!-- <a @click="goItem(p)" av-link="{name: 'post-detail', params:{detailId: p.fileName}}">{{p.title}}</a> -->
      </li>
    </ul>
  </div>
</div>
...
export default {
  ...
  methods: {
    goItem(p) {
      ...
      // this.$route.router.push({name: 'post-detail', params:{detailId: p.fileName}})
    },
  ...
  mounted() {
    eventBus.emit('stopPropagation', this.$el)
  }
}
</script>
